...
impl DB {
	...
	pub async fn create_note(&self, body: &CreateNoteSchema) -> Result<SingleNoteResponse> {
		...
		let insert_results = match self.collection.insert_one(&document, None).await {
			Ok(result) => result,
			Err(e) => {
	...
	pub async fn get_note(&self, id: &str) -> Result<SingleNoteResponse> {
		...
		match note_doc {
			Some(doc) => {
				...
				Ok(SingleNoteResponse { status: "success", data: NoteData { note } })
			},
